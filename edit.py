...
st.set_page_config(page_title="Masher — Video Editor", page_icon="🎬", layout="wide")
...
:root{
  --bg:#0c0c0c;--surface:#161616;--border:rgba(255,255,255,0.08);
  --border-md:rgba(255,255,255,0.15);--text:#f0ece4;--muted:#9a9590;
  --faint:#5a5650;--accent:#c8102e;--accent-bright:#e8192e;
  --tag-bg:#1e1e1e;--radius:12px;--radius-sm:8px;
...
nav{
  ...
  padding:1.1rem 2.5rem;background:rgba(12,12,12,0.90);
  backdrop-filter:blur(14px);border-bottom:0.5px solid var(--border);
}
.logo{font-family:var(--font-display);font-size:20px;letter-spacing:4px;color:var(--text);text-decoration:none}
.logo span{color:var(--accent)}
.nav-links{display:flex;gap:2rem;list-style:none}
.nav-links a{font-size:13px;color:var(--muted);text-decoration:none;letter-spacing:0.4px;transition:color .2s}
.nav-links a:hover{color:var(--accent-bright)}
/* HERO */
.hero{padding:6rem 2.5rem 5rem;max-width:860px;animation:fadeUp .7s ease both;position:relative;z-index:1}
.hero-eyebrow{display:inline-flex;align-items:center;gap:8px;font-size:11px;letter-spacing:2.5px;color:var(--muted);text-transform:uppercase;margin-bottom:1.4rem}
.dot{width:6px;height:6px;border-radius:50%;background:var(--accent);display:inline-block;animation:pulse 2s ease-in-out infinite;box-shadow:0 0 8px var(--accent)}
@keyframes pulse{0%,100%{box-shadow:0 0 6px var(--accent)}50%{box-shadow:0 0 14px var(--accent-bright)}}
.hero h1{font-family:var(--font-display);font-size:clamp(38px,6vw,60px);font-weight:400;line-height:1.1;color:var(--text);margin-bottom:1.4rem}
.hero h1 em{color:var(--accent);font-style:italic}
.hero p{font-size:16px;color:var(--muted);max-width:620px;margin-bottom:2.2rem;line-height:1.85}
.btn-row{display:flex;gap:12px;flex-wrap:wrap}
.btn-dark{
  padding:11px 24px;font-size:13px;font-weight:500;background:var(--accent);color:#fff;
  border:none;border-radius:100px;cursor:pointer;font-family:var(--font-body);letter-spacing:0.3px;
  text-decoration:none;display:inline-block;transition:background .2s,box-shadow .2s;
...
.section-label{font-size:11px;letter-spacing:2.5px;color:var(--accent);text-transform:uppercase;margin-bottom:1.4rem;opacity:0.8}
/* TAGS */
.tags{display:flex;gap:8px;flex-wrap:wrap}
.tag{
  font-size:13px;padding:6px 16px;background:var(--tag-bg);border:0.5px solid var(--border);
  border-radius:100px;color:var(--muted);display:flex;align-items:center;gap:6px;
  transition:border-color .2s,color .2s;
}
.tag:hover{border-color:var(--accent);color:var(--text)}
.tag i{font-size:14px}
/* WORK GRID */
.works-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:14px}
.work-card{
...
.work-card:hover{transform:translateY(-3px);box-shadow:0 8px 32px rgba(200,16,46,0.15);border-color:rgba(200,16,46,0.35)}
.work-thumb{height:130px;display:flex;align-items:center;justify-content:center;position:relative;overflow:hidden}
.thumb-youtube{background:#1c1010}
.thumb-wedding{background:#121218}
.thumb-reel{background:#101218}
.thumb-event{background:#181012}
.work-thumb i{font-size:36px;color:rgba(200,16,46,0.35);transition:color .2s}
...
.ext-badge{
  ...
  font-size:10px;letter-spacing:1px;padding:3px 8px;border-radius:4px;text-transform:uppercase;
}
...
.play-btn{
  ...
  margin:0 auto 1.2rem;cursor:pointer;transition:transform .2s,box-shadow .2s;
  border:none;text-decoration:none;box-shadow:0 0 24px rgba(200,16,46,0.45);
}
...
.tool-chip{
  font-size:12px;padding:6px 14px;background:var(--surface);border:0.5px solid var(--border);
  border-radius:var(--radius-sm);color:var(--muted);transition:border-color .2s,color .2s;
}
.tool-chip:hover{border-color:var(--accent);color:var(--text)}
/* CONTACT */
.contact-grid{display:grid;grid-template-columns:1fr 1fr;gap:2rem}
@media(max-width:600px){.contact-grid{grid-template-columns:1fr}}
.contact-info h3{font-family:var(--font-display);font-size:26px;font-weight:400;margin-bottom:.8rem;color:var(--text)}
...
#formStatus{font-size:13px;text-align:center;display:none;margin-top:4px}
/* FOOTER */
footer{
  padding:1.5rem 2.5rem;border-top:0.5px solid var(--border);
  display:flex;justify-content:space-between;align-items:center;
  flex-wrap:wrap;gap:12px;position:relative;z-index:1;
}
footer span{font-size:12px;color:var(--faint)}
.social-row{display:flex;gap:16px}
.social-row a{font-size:20px;color:var(--faint);text-decoration:none;transition:color .2s}
...
<section class="hero">
  <div class="hero-eyebrow"><span class="dot"></span> Available for projects · Based in India</div>
  ...
  <div class="btn-row"><a href="#contact" class="btn-dark">Work with me</a></div>
</section>
...
<section class="section" id="work">
  ...
  <div class="works-grid">
    <a class="work-card" href="https://youtu.be/5bNp0VifFq4?si=IA6nVr3rOyP4SdM5" target="_blank" rel="noopener">
      <div class="work-thumb thumb-youtube"><i class="ti ti-brand-youtube"></i><span class="ext-badge">YouTube</span></div>
      <div class="work-info"><p class="work-title">YouTube Travel Vlog</p><p class="work-meta">YOUTUBE · VLOG</p></div>
    </a>
    <a class="work-card" href="https://www.instagram.com/reel/DXvOVCGJ8US/?utm_source=ig_web_copy_link&igsh=MzRlODBiNWFlZA==" target="_blank" rel="noopener">
      <div class="work-thumb thumb-reel"><i class="ti ti-brand-instagram"></i><span class="ext-badge">Reel</span></div>
      <div class="work-info"><p class="work-title">Brand Instagram Reel</p><p class="work-meta">SOCIAL MEDIA · 30 SEC</p></div>
    </a>
    <div class="work-card">
      <div class="work-thumb thumb-wedding"><i class="ti ti-heart"></i></div>
      <div class="work-info"><p class="work-title">Wedding Highlight Film</p><p class="work-meta">WEDDING · 4 MIN</p></div>
    </div>
    <div class="work-card">
      <div class="work-thumb thumb-event"><i class="ti ti-confetti"></i></div>
      <div class="work-info"><p class="work-title">Birthday Event Film</p><p class="work-meta">EVENT · 6 MIN</p></div>
    </div>
  </div>
</section>
